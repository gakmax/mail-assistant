...
- **3열을 기본값으로** — 390px에는 추이 차트도 브리핑도 못 산다. `grid(minimum=240)`이
  1단으로 접혀 두 막대 카드가 세로로 쌓인다. 고를 수는 있게 하되 기본은 2열이다
...
- **우선순위를 합산(누적) 막대로** — 순서가 있고 합이 전체라 어울릴 것 같지만
  `보통 #4b5563`과 `낮음 #808080`의 인접 ΔE가 15.6으로 정상 시야 하한 15를 겨우 넘는다.
  막대 넷으로 떨어져 라벨이 붙어 있을 때는 문제가 없고 한 막대 안에서 맞붙으면 경계가
  사라진다. 회색 둘은 실수가 아니라 긴급·높음만 눈에 걸리게 하려는 의도다
- **도넛** — 0건은 조각이 없어 그릴 수도 누를 수도 없다. 이 앱이 피하려는 실패 그 자체다
