...
### 10단계 — 배치 선택과 카드 재배치 (진행 중)

"대시보드 요소를 사용자가 직접 옮길 수 있나"에서 출발했다. 답은 못 옮긴다였고, 막고
있는 것은 드래그 코드가 없다는 사실이 아니라 **두 열의 폭이 다르다**는 것이다.
`.ma-split`은 `minmax(0,1.35fr) minmax(0,1fr)`이라 왼쪽은 차트용, 오른쪽은 패널용으로
전제가 갈려 있다. 카드를 옮기면 옮긴 자리에서 다르게 보이므로, 폭을 맞추기 전에는
재배치가 기능이 아니라 고장이다.

`.ma-page`는 `max-width:1240px`에 좌우 20px이므로 내용 폭 1200px, 사이드바를 펼치면
(216px) `WINDOW` 1440에서 1224에 걸려 1184px이다. `gap:14px`을 빼면:

| | 왼쪽 | 오른쪽 |
|---|---|---|
| 현재 `1.35fr : 1fr` | 681px | 505px |
| 균등 `1fr : 1fr` | 593px | 593px |
| 균등 3열 | 390px (펼치면 385px) | |

- [x] **10.1 균등 2열** — `.ma-split`을 `minmax(0,1fr) minmax(0,1fr)`로. 593px에서도
      두 막대 카드의 `grid(minimum=240)`은 2단(289px)을 유지하고, 추이 차트는
      `label_step()`이 x축 라벨을 솎으므로 버틴다. 오른쪽 패널은 넓어지는 쪽이다
- [x] **10.2 설정 기어** — 대시보드 우상단에 `ui.menu` 하나. 열 비율(1:1 / 1:1:1)과
      종류·우선순위 차트 형태를 고른다. 상태는 `app.storage.user['home']`.
      **10.2에서는 열 배치만 넣었다** — 차트 형태는 10.3이 그 구현을 만든 뒤에야 고를 것이
      생긴다. 실측: 1440 창에 사이드바를 펼치면 2열 585px, 3열 385px로 계획한 값과 같고,
      캔버스는 이동 뒤 `run_chart_method('resize')`로 347px에 다시 맞춰졌다
- [x] **10.3 메일 종류·우선순위를 DOM 행으로** — 캔버스를 걷어내고 `<a>` 줄로 그린다.
      기어에서 목록/막대를 고르고 기본은 목록이다. 실측: 목록에서 대시보드의 캔버스가
      3개에서 1개(추이)로 줄고, 0건인 공지·긴급이 28px 높이의 링크로 남아 눌러서
      `/mail?category=공지`가 열렸다
- [x] **10.4 카드 드래그 재배치** — 열 안 순서와 열 간 이동. 그립 8개, 실제 브라우저
      드래그로 종류·우선순위 카드를 1열에서 2열 맨 위로 옮기고 새로고침 뒤에도 유지됐다

#### 되짚어 둘 것

**열 비율은 CSS지만 열 개수는 아니다.** 1.35:1 ↔ 1:1은 `grid-template-columns` 한 줄이라
리빌드가 없고 캔버스도 살아 있다. 3열은 카드가 열을 넘어가야 하므로 다르다 — 다만
nicegui 3.16의 `Element.move(target_container, target_index)`가 **있던 엘리먼트를 옮기기**
때문에 여기서도 리빌드는 필요 없다. 옮긴 뒤 폭이 바뀐 차트에만
`run_chart_method('resize')`를 부른다. 캔버스는 컨테이너 폭이 변해도 스스로 다시 재지
않으며, 창 크기가 그대로면 resize 이벤트도 오지 않는다.

**드래그의 앵커는 카드가 아니라 refreshable의 컨테이너다.** 대시보드는
`REFRESH_SECONDS`마다, 그리고 `Hub.revision`이 움직이면 즉시 다시 그린다. `@ui.refreshable`
호출은 `RefreshableContainer` 엘리먼트를 하나 만들고 `refresh()`는 그 **자식만** 지웠다
다시 만들므로, `today_block()`이 만드는 `.ma-card`는 5초마다 사라지지만 컨테이너는 남는다.
드래그와 드롭 대상은 그 컨테이너여야 한다. 카드에 걸면 비트 하나가 포인터 밑에서 끌던
것을 지운다. 칸반이 이 문제를 겪지 않은 이유는 `lanes()`가 쓰기 때만 다시 그리기
때문이지, 드래그가 원래 안전해서가 아니다.

**카드 전체를 `draggable`로 만들면 안 된다.** 칸반 카드는 글자 덩어리라 통째로 끌어도
됐지만 대시보드 카드 안에는 마감 체크박스, 7/14/30 토글, KPI 링크가 있다. 카드 머리에
그립(`drag_indicator`)만 `draggable=true`로 두고 드래그 이미지는 `setDragImage`로
카드를 씌운다.

**`js_handler` 규칙은 칸반과 같고, 여기서 더 세다.** `dragover`/`dragleave`/`dragstart`/
`dragend`는 전부 `js_handler`, `emit`은 `drop` 한 번. 열 안 **순서**까지 정하려면 카드마다
`dragover`가 붙으므로, 파이썬 핸들러를 하나라도 주면 카드 한 장이 1200px을 건너가는 동안
웹소켓 메시지가 수백 개 간다.

**열 개수별로 순서를 따로 들고 있어야 한다.** `order['2']`와 `order['3']`. 하나만 두면
2열↔3열을 오갈 때마다 배치가 섞인다. 모르는 키는 버리지 말고 뒤에 붙인다 — Codex가 더는
나열하지 않는 모델의 행을 `model_rows()`가 남기는 것과 같은 이유로, 나중에 추가된 카드가
저장된 순서에 없다고 화면에서 사라지면 안 된다.

**기어는 헤더 띠가 아니라 페이지에 둔다.** 0.8.2에서 접기 버튼을 띠에서 뺀 이유가 그대로
적용된다: 띠에 두면 열 개 페이지 전부에 뜨는데 동작은 대시보드에서만 한다. 브리핑 위에
얇은 줄을 만들거나, 지금 맨 아래 있는 `summary_row()`를 위로 올려 그 줄 오른쪽 끝에
붙인다(세로 공간이 공짜가 된다). 메뉴 자체는 `window`·`log_open`처럼 `home()`이 쥐고
refreshable 밖에 산다.

**막대의 *형태*는 이미 맞는 답이었다.** 명목 범주 여섯 개의 크기 비교는 가로 막대에 전부
한 색이고, `bar_option`이 `SERIES` 하나로 칠하는 것이 그것이다. 크기에 따라 색을 진하게
하는 것은 막대 길이를 두 번 부호화하는 안티패턴이다. 바꾸는 것은 형태가 아니라 **구현**이다:
`yAxis.triggerEvent`·`BAR_EVENT`·`clicked_bar()`는 전부 "0건은 막대가 없어 11px 축 라벨이
유일한 클릭 영역"이라는 문제를 우회하려고 있고, 줄 전체가 `<a>`이면 그 우회가 통째로
없어진다. 다만 **세 헬퍼가 저장소에서 사라지지는 않는다** — 통계 화면이 같은 두 차트를
같은 `bar_link`로 그리고 있다. 그러니 기어에 막대를 남기는 값은 거의 공짜이고, 고르게
한다. 기본값은 목록이다. 대시보드에서 캔버스가 추이 하나로 주는 것은 그대로라, 10.4의
`resize` 부담도 같이 준다.

#### 하지 않기로 한 것

- **3열을 기본값으로** — 390px에는 추이 차트도 브리핑도 못 산다. 고를 수는 있게 하되
  기본은 2열이다
- **CSS multicol(`column-count`)로 열 개수 바꾸기** — DOM을 안 건드리고 N열이 되지만
  브라우저가 높이로 균형을 맞추므로 카드가 어느 열에 앉을지 정할 수 없고, 5초마다
  높이가 변하는 이 페이지에서는 카드가 열 사이를 저절로 건너다닌다
- ~~**우선순위를 합산(누적) 막대로**~~, ~~**도넛**~~ — **요청을 받아 10.5에서 넣었다.**
  아래에 조건을 적었다
- **막대를 건수 내림차순으로 정렬** — 더 빨리 읽히는 것은 맞지만 이 카드는 5초마다 다시
  그려지므로 순서가 바뀌면 줄이 매번 뛴다. `CATEGORIES` 고정 순서는 위치를 외울 수 있게
  해 주는 값이다

### 10.5단계 — 합산·도넛, 그리고 전폭 (완료)

10단계에서 접었던 두 형태를 요청을 받아 넣었다. 접었던 이유는 사라지지 않았으므로
**조건을 붙여서** 넣는다 — 그 조건이 이 절의 내용이다.

- [x] **1:1에서 메일 종류·우선순위가 한 칸을 다 쓴다.** 둘을 반씩 나눠 쓰던
      `grid(minimum=240)` 묶음을 없앴다. 실측 585px씩
- [x] **그래서 둘을 따로 움직이는 카드로 쪼갰다.** `counts` 한 덩어리였던 것이
      `kinds`와 `ranks`가 되어 `HOME_BLOCKS`가 9개다. 전폭이 된 뒤에도 한 단위로 묶어
      두면 하나를 끌 때 다른 하나가 딸려온다
- [x] **합산 막대와 도넛** — `COUNT_FORMS`가 넷이 되고, 캔버스를 쓰는 둘은
      `CANVAS_FORMS`로 묶어 타이머가 옵션만 덮어쓰게 했다

**0건은 범례가 받는다.** 두 형태 모두 0을 그릴 수 없다는 사실은 그대로다 — 조각이
없으니 누를 것도 없다. 그래서 그림 아래에 `count_legend()`를 두고, 이름·건수·비율을
가진 줄이 **각각 `<a>`**가 된다. 0건인 줄에는 **그림에 없음** 표가 붙는다. 그림이
못 하는 말을 줄이 대신 하는 구조이고, 이것이 없으면 두 형태는 10.3이 없애려던 실패로
그대로 돌아간다.

**두 회색은 2px 틈이 갈라 준다.** `보통 #4b5563`과 `낮음 #808080`은 인접 ΔE 15.6으로
붙여 놓으면 경계가 사라진다. `.ma-comp`의 `gap:2px`와 도넛의 `padAngle:1.5`가 그
경계이며, 장식이 아니라 구조다.

**메일 종류에는 팔레트가 없어서 만들었다.** 우선순위는 `STATUS`가 있지만 그것은
*상태* 팔레트라 아무 상태도 아닌 범주에 빌려 쓸 수 없다. 처음에는 파랑 6단계 램프로
했는데 검증기가 두 군데서 떨어뜨렸다 — 공지와 기타가 **정상 시야 ΔE 7.6**(하한 15),
여섯 중 넷이 카드 대비 **3:1 미만**. 그래서 `SEGMENT_TONES` 여섯 색을 범주 팔레트로
만들고, **이웃한 쌍이 약한 쌍이 되지 않도록 순서까지 정했다**. 처음 넣은 값은 대비를
전부 넘기려고 각 색조의 어두운 끝을 골라 탁했고, 그 지적을 받아 **파스텔 계열로
바꿨다**(최악 인접 20.3 정상 / 18.9 deutan). 여섯 중 셋이 3:1 아래인데, 그림이 유일한
라벨이 아닐 때만 허용되는 조건이라 **범례가 그 조건이다** — 두 형태를 범례 없이 그리면
0건과 대비가 한꺼번에 깨진다. 이웃이 아니라 *모든* 쌍으로 재면 떨어진다(파랑 둘이
ΔE 6.3). 다만 여섯 색짜리는 전부 떨어지고 그 전에 출하한 값도 deutan 0.4였다 —
all-pairs는 산점도·코로플레스용이고 3개까지가 상한이다. 색은
`CATEGORIES`의 고정 위치로 나눠 주며 건수 순서로 주지 않는다 — 순위로 주면 크기를 두 번
부호화하고 숫자가 움직일 때마다 색이 갈아엎힌다. 테스트가 같은 산수를 다시 돌린다.
`style.py`를 거치지 않는 두 번째 팔레트이고, 첫 번째가 메모 종이인 것과 같은 이유다 —
창에는 합산도 도넛도 없어서 어긋날 상대가 없다.

- [x] 화면 단위 테스트 445개

#### 구현하면서 배운 것 두 가지

**`THEME`에서 클래스 이름이 겹치면 조용히 진다.** 카운트 줄을 `.ma-tally`로 지었는데 그
이름은 할 일 판 집계가 이미 쓰고 있었다. 한 장짜리 스타일시트라 뒤에 있던
`display:flex`가 앞의 `display:grid`를 이겼고, 줄이 세로로 쌓이는 대신 한 줄에 나란히
붙었다 — 화면을 열기 전에는 아무것도 말해 주지 않았고 테스트도 `.ma-tally__row`가
THEME에 있는지만 보고 통과했다. `ThemeCollisionTests`가 이제 같은 선택자에 `display`가
두 번 선언되면 빌드를 깨뜨린다. 중복 선택자 전체가 아니라 `display` 하나만 보는 이유는
`.ma-sheet td, .ma-sheet th` 뒤에 `.ma-sheet th`로 좁히는 것은 정당한 패턴이기 때문이다.

**드롭 표시선이 지워지지 않았다.** 슬롯의 drop이 `stopPropagation`을 부르므로 그 아래
`.ma-stack`은 `dragover`에서 `is-over`를 붙여 놓고 drop을 못 듣는다. 드래그가 어떻게
끝나든 반드시 한 번 도는 이벤트는 `dragend`뿐이라, 쓸어내기는 거기(`SLOT_END`)에 있다.

#### 작업 순서

10.1 + 10.2를 먼저(CSS·`Element.move()`·`app.storage.user`), 10.3을 그다음, 10.4는 별개
착륙으로 둔다. 10.3이 먼저인 이유는 그것을 끝내면 10.4가 다뤄야 할 캔버스가 하나로 줄기
때문이다. — **넷 다 끝났다(0.10.0).** 화면 단위 테스트 422개.

이 작업은 "`webui.py`를 쪼갤지"(아래 미해결 질문)를 더 밀어붙인다. 설정 메뉴·배치 상태·
드래그가 전부 그 파일로 들어간다.
