...
### 넘겨야 할 결정 하나 — 0.4.0에서 받았다

**웹 화면을 기본 창으로 삼기로 했다.** `MailAssistant.exe`가 pywebview 창에 화면들을
띄우고, 바로가기와 `--autostart` 경로는 손대지 않았다 — 같은 exe가 다른 창을 열 뿐이다.
종료 의미도 그대로다: native 창을 닫으면 `ui.run`이 반환되고 `finally`의 뮤텍스 해제 →
설치 프로그램 실행 순서가 유지된다. 업데이트 화면은 `updater.py` + 현황의 다이얼로그와
실행 화면의 카드로 옮겼다(5단계 미완 항목 해소).

**`app.py`는 지우지 않았다.** `--window`와, pywebview를 못 불러오는 PC의 폴백이다. CI가
native 창을 띄워보지 못하므로 그런 PC에서 사람이 검증할 수 있는 유일한 경로이기도 하다.
tkinter를 들어내면 tcl/tk 3.7MB가 빠져 순증이 +0.6MB가 되지만, native가 현장에서 검증된
뒤에 할 일로 남긴다.

### 미해결 질문

- [x] native 모드(pywebview 창) vs 기본 브라우저 — **native로 결정(0.4.0)**. 실측 +4.3MB는
      pywebview가 Windows에서 WebView2(이미 깔린 Edge 엔진)를 빌려 쓰기 때문이며, 크롬을
      동봉하는 Electron(+120MB대)과는 다른 비용이다. 창을 닫으면 프로세스가 끝나므로
      트레이 아이콘도 필요 없어졌다
- [x] 아홉 개 내비게이션을 어디에 둘지 — **0.6.0에서 왼쪽 사이드바로 옮겼다.** 결정한 것은
      취향이 아니라 산수다: 헤더 띠의 `.ma-bar__inner`는 `max-width:1240px`인데 브랜드
      130px + 링크 아홉 개 670px로 이미 860px을 쓰고 있었고, `WINDOW_MIN`인 1080 창에서
      로고·상태·업데이트·뒤로가기를 더하면 넘친다. `.ma-nav`가 `overflow-x:auto`에
      `scrollbar-width:none`이라 넘치는 순간 설정 탭이 **스크롤바도 없이** 잘려 나간다.
      그래서 왼쪽은 '어디로', 위는 '지금 어떤 상태인가'로 갈랐다. 세로로 서면서 생긴 자리에
      배지가 들어갔고(메일·할 일·초안), 숫자는 `overview()`/`board_counts()`를 통과해
      대시보드 카드와 같은 값이다. 1200px 미만에서는 아이콘 레일(64px)로 접히고 배지는 점이
      된다 — 메일 표의 여섯 열이 눌리는 폭이 거기다.
      **값을 치른 곳:** `.ma-main`을 flex column으로 두었다가 `.ma-page`의 `margin:0 auto`가
      flex 아이템의 교차축 auto 마진이 되어 페이지가 내용 너비로 쪼그라들었다(카드 네 장이
      3+1로 줄바꿈). 블록으로 되돌렸고 CLAUDE.md에 남겼다
- [ ] 트레이 아이콘을 넣을지(창 닫기 = 트레이로 숨김) — **0.6.0에서 다시 물었고, 보류.**
      ...
      **0.6.0에서 다시 보며 다른 길을 하나 찾았고, 그 길은 접었다.** `ui.run(frameless=True)`로
      OS 타이틀바를 없애고 최소화·최대화·닫기를 우리 HTML로 그리면 X가 우리 버튼이 되므로
      `closing` veto 자체가 필요 없어진다 — 창을 숨기는 것은 `app.native.main_window.hide()`
      이고 `WindowProxy`에 이미 중계돼 있으며, 창 프로세스가 살아 있는 한 nicegui의
      `check_shutdown`은 서버를 내리지 않는다. 트레이 아이콘도 부모 프로세스에 그대로 둘 수
      있다. **그런데 트레이만이 목적이라면 이쪽이 더 싼 길이 아니다.** frameless는
      `FormBorderStyle.None`(pywebview `winforms.py`)이고 패키지 어디에도 `WM_NCHITTEST`가
      없어 가장자리 리사이즈가 사라진다. `WS_THICKFRAME`을 `SetWindowLong`으로 되붙이면
      돌아오지만(`GWL_STYLE`은 프로세스 경계를 넘는다 — 같은 프로세스여야 한다는 제약은
      `GWLP_WNDPROC`에만 있다), 그때 생기는 비클라이언트 테두리가 흉하면 `WM_NCCALCSIZE`가
      필요하고 그건 다시 창 프로세스 안이다. 드래그-투-스냅과 Win11 스냅 레이아웃은
      `HTCAPTION`이 없어 영구히 돌아오지 않는다.
      **결론:** OS 타이틀바 대체는 접는다 — 얻는 것이 미관뿐인데, 실패하면 '창이 이상해진다'는
      눈에 보이는 회귀이고, 릴리스마다 사람이 창을 끌고 늘려 보는 수동 검증 목록이 영구히
      생긴다. 트레이를 하게 되면 원래의 `start_args['func']` veto로 한다 — 실패해도 오늘
      동작(X = 종료)으로 돌아갈 뿐이고, `FormClosing`이 Alt+F4와 작업표시줄 닫기까지 함께
      잡아 오히려 더 완전하다. 헤더가 원하던 것(로고·상태·업데이트·뒤로가기)은 창 프레임과
      무관했고, 위 항목에서 HTML로 끝냈다
