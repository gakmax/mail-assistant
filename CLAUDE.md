...
**시트의 CTA 는 48px 이 아니다 — 48px 은 엄지의 치수다.** TDS 의 48px CTA 는 한 손으로
드는 전화기의 계산이고, 이 창은 1440px 짜리 화면 한가운데 520px 로 떠 있다. 그 안에서
48px/17px 짜리 버튼 둘은 폼 전체보다 무겁게 읽혔다 — `.ma-ask__cta .q-btn` 은 M(40px,
`--fs-b2`, `--r-m`)이고, 그것이 이 앱의 일하는 크기다. 창 밖의 저장 버튼들이 이미 그
크기이고, 전수로 재 보면 이 앱의 버튼은 상담의 보내기 하나만 빼고 전부 32px 이다.
`취소`는 그래도 `fill-secondary` 로 남는다 — 바탕 없는 버튼은 40px 에서도 여백으로 읽힌다.
그리고 창 안의 설명은 위계가 셋이어야 한다: 제목 `--fs-t1` · 라벨 `--fs-b3`(굵게) ·
설명 `--fs-cap`(회색). `.ma-ask__sub` 가 `--fs-b2` 였을 때 제목과 한 칸밖에 차이가 나지
않았고, `say()` 가 라벨과 같은 크기였을 때는 어느 쪽이 묻는 말인지가 크기로 말해지지
않았다.

**버튼의 크기를 정하는 것은 높이가 아니라 그 안의 아이콘이다.** Quasar 는 아이콘을 버튼
글자의 1.715배로 그리므로, 손대지 않으면 13px 짜리 라벨 옆에 20px 이 서고 32px 짜리 동그란
버튼은 그 하나로 칸의 62%가 찬다. '버튼이 크다'는 실은 그 비율이고, `--ic-*` 가 본문
램프와 따로 있는 이유가 여기에 있다 — 버튼의 크기가 곧 아이콘의 칸이다: `.q-btn`(40px)은
`--ic-m`, `.q-btn--dense`(32px)는 `--ic-s`. 이 둘을 고치면 사이트 전체의 버튼이 함께
가벼워지므로, 어느 한 버튼을 따로 줄이는 것보다 먼저 볼 곳이다. 한 가지 덤: `.q-btn` 이
제 `transition` 을 갖게 되었으므로 **제 transition 을 가진 `.ma-*` 버튼은 이름에 `.q-btn`
을 붙여야 한다** — 한 클래스끼리는 뒤가 이기고 `.q-btn` 이 이 파일에서 더 뒤에 있어서,
`.ma-said__copy` 와 `.ma-side__fold` 의 opacity 가 소리 없이 계단을 잃었다.

**hover 가 다시 칠할 때는 시간이 있어야 한다.** 스무 규칙이 `:hover` 에서 바탕이나 글자
색을 바꾸면서 그 변화에 시간을 주지 않고 있었다 — 포인터 밑의 땅이 한 프레임에 회색이
되었다가 한 프레임에 돌아오는 것은 반응이 아니라 깜빡임이고, 이것이 가장 비싼 곳은 읽는
사람의 포인터가 하루를 보내는 두 군데였다: 메일 목록의 모든 행과 대시보드의 집계 줄 넷·
패널 둘. 규칙은 '모든 규칙에 transition' 이 아니라, **hover 에서 변하는 선택자가 여행할 수
있어야 한다** — 그 선택자의 쉬는 규칙이(또는 같은 이름을 이미 쓰는 규칙이) transition 을
들고 있어야 한다. `HoverMotionTests` 가 이것을 붙들고 있고, 선택자의 *마지막* compound 를
보는 것이 그 테스트의 요점이다: `.ma-memo:hover .ma-memo__acts` 가 움직이는 것은
`.ma-memo__acts` 이므로 transition 도 거기 있어야 한다.

**날짜는 브라우저에게 묻지 않는다.** `date_field()` 는 `type=date` 가 있던 자리다. 그것을
두지 않는 이유는 모양이 아니라 말이다: WebView2 가 그리는 그것은 `mm/dd/yyyy` 로 묻고
달력은 영어로 열리는데, 이 앱이 날짜를 적는 자리는 전부 `2026-09-25` 다 — 한 폼 안에서 한
값을 두 가지로 쓰면 읽는 사람이 어느 쪽이 달인지 세어야 한다. 그리고 그것은 `THEME` 이
닿지 않는 유일한 표면이라, 창 안에서 저 혼자 다른 앱의 부품처럼 보인다. 대신 있는 것은
마스크 낀 `ui.input` 하나와 그 옆 아이콘이 여는 `ui.date`(QDate) 하나이고, 값은 양쪽 다
`YYYY-MM-DD` 이므로 `stamp()`·`add_event()`·`add_money()` 는 아무것도 달라지지 않았다.
지키는 것 다섯. **고르는 것과 적는 것 중 하나를 고르지 않는다** — 달력은 아이콘이 열고
(`no-parent-event`), 칸 자체는 늘 받아쓴다: 2026-09-25 를 아는 사람이 달력을 세 번 눌러
찾는 것은 도와주는 것이 아니다. **주는 월요일부터 센다**(`CAL_LOCALE['firstDayOfWeek']`),
일정 화면의 FullCalendar 가 `firstDay:1` 이기 때문이고, 요일 이름은 `WEEKDAYS` 에서 한 번만
옮긴다 — QDate 의 locale 은 일요일부터 세므로 그 순서를 뒤집는 곳이 둘이 되면 9월이 두
가지 모양이 된다. **`CAL_WIDTH` 는 필요하다**: QDate 의 칸들은 flex row 라 자리가 있는
만큼 벌어지고, 폭을 주지 않으면 창 안에서 시트보다 넓은 달력이 열린다. **여섯 줄 자리는
비워 둔 채로 둔다** — 3월처럼 여섯 줄이 필요한 달로 넘어갈 때 창이 자라면 QMenu 는 이미
자리를 잡은 뒤라 다시 잡지 않고 화면 아래로 삐져나간다(고칠 것은 그 예비 높이의 *크기*
였다: Quasar 의 것은 제 44px 짜리 줄로 잰 것이라 `CAL_ROW` 로 줄인 이 격자에서는 쓰이지
않는 한 줄 반이 남았다). 그리고 **발밑에는 격자가 닿지 못하는 값만 둔다** — 오늘과 빈 값
둘뿐이고, 물러나는 길은 따로 두지 않는다: 바깥을 누르거나 Esc 를 누르면 닫히고 그것이 이
창에 대한 모든 사람의 기대다. 시각 칸은 같은 모양의 마스크 낀 칸이고 문자판이 없다 —
14:00 을 적는 편이 언제나 빠르고, 고친 것은 `--:-- --`(오전/오후)이지 위젯이 아니었다.

**메일 붙이기의 칸은 글자마다 다시 만들어지지 않는다.** `panel.refresh()` 가 찾는 칸과
찾은 줄을 한꺼번에 지웠고, 새로 만들어진 칸에는 캐럿이 없다 — 한 글자 적을 때마다 포커스가
빠지는 칸이었다. 고친 것은 둘이고 둘 다 필요하다. 안쪽에 `hits()` 를 따로 두어 `look()`
이 찾은 줄만 다시 그리고(중첩된 refreshable 은 nicegui 가 `prune()` 으로 지워진 컨테이너를
걷어 가므로 `panel` 이 다시 그려져도 겹치지 않는다), 서버가 그 말을 듣는 것을 `PICK_WAIT`
동안 미룬다 — Quasar 의 `debounce` 는 모델 갱신 자체를 늦추므로, 그 사이에는 서버가 이 칸의
존재조차 모른다. 이 두 가지가 이 앱에서 '입력하는 동안 찾는' 유일한 칸의 규칙이고, 같은
모양을 하나 더 만들 때 베낄 것도 이것이다.

**`.ma-menu` 는 제 안에 있는 것만큼 넓다.** 대시보드의 화면 배치 메뉴가 `min-width:184px`
안에서 네 칸짜리 토글을 줄바꿈해 '목록'이 '목/록'으로 뭉개졌다. 고친 것이 더 큰 숫자가 아닌
이유는 한 칸의 폭을 정하는 것이 글꼴이기 때문이다 — Pretendard 가 늦게 오면 이 앱은 맑은
고딕으로 한 번 그려지고(`FONT_PRELOAD` 가 있는 이유와 같은 사정), 그때 맞는 숫자는 다른
숫자다. `width:max-content` 는 어느 글꼴에서도 맞고, 짝이 되는 절반은 `.ma-seg .q-btn` 의
